...
const managers = require('../../../../model/manager/ragister');
...
module.exports.profile = async (req, res) => {
    ...
    try {
        let managerdatas = await ragister.findById(req.user.id).populate('adminids').exec();
        if (managerdatas) {
            return res.status(200).json({ msg: 'Data Found Succ....', status: 1, rec: managerdatas });
        }
        else {
